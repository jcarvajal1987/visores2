import { c as createAstro, b as createComponent, r as renderTemplate, e as addAttribute, f as renderHead, g as renderSlot, h as renderComponent, m as maybeRenderHead, u as unescapeHTML } from './astro_f59ab796.mjs';
/* empty css                         */
...
Connotación Neutral
</div>`}</div>${post.tipo === 3 && renderTemplate`<div class="container grid"><video id="2" class="video start1 invisibility print-visibility "${addAttribute(post.url_img, "src")}></video><video controls autoplay id="2" class="video start1 print-block print-invisibility"${addAttribute(post.url_img, "src")} webkit-playsinline playsinline></video></div>`}${post.tipo == 4 && renderTemplate`<div class="container grid"><audio controls><source${addAttribute(post.url_img, "src")} type="audio/mpeg"></audio></div>`}${post.tipo === 1 && renderTemplate`<div class="title-papel">${post.titulo}</div>`}${post.tipo === 2 && renderTemplate`<div class="title">${post.titulo}</div>`}${[3, 4].indexOf(post.tipo) > -1 && renderTemplate`<div class="title-video">${unescapeHTML(post.titulo)}</div>`}${post.tipo === 1 && renderTemplate`<div class="cachetxt-papel">${unescapeHTML(post.cachetxt)}</div>`}${(post.tipo === 2 || 4) && renderTemplate`<div class="cachetxt">${unescapeHTML(post.cachetxt)}</div>`}</div></div><div class="hidden-pdf xs pointer detail-layout"><div id="SidebarToggle" class="abrir flex">
